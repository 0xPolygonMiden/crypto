use super::{
    BTreeMap, EmptyNodesSubtrees, MerkleError, MerklePath, NodeIndex, Rpo256, RpoDigest, Vec, Word,
};
...
impl SimpleSmt {
    ...
    {
        self.store
            .replace_empty_hashes(hashes.into_iter().collect());
        self
    }
...
impl Store {
    fn new(depth: u8) -> (Self, Word) {
        ...
        let empty_hashes = EmptyNodesSubtrees::compute_empty_hashes(depth);
    ...
    fn replace_empty_hashes(&mut self, hashes: Vec<RpoDigest>) {
