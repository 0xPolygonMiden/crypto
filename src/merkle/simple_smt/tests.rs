...
fn build_sparse_tree() {
    ...
    assert_eq!(
        ...
        smt.open(LeafIndex::<3>::new(6).unwrap())
    );
    ...
    assert_eq!(
        ...
        smt.open(LeafIndex::<3>::new(2).unwrap())
    );
    assert_eq!(old_value, EMPTY_WORD);
}
...
fn test_depth2_tree() {
    ...
    assert_eq!(vec![VALUES4[1], node3], *tree.open(LeafIndex::<2>::new(0).unwrap()));
    assert_eq!(vec![VALUES4[0], node3], *tree.open(LeafIndex::<2>::new(1).unwrap()));
    assert_eq!(vec![VALUES4[3], node2], *tree.open(LeafIndex::<2>::new(2).unwrap()));
    assert_eq!(vec![VALUES4[2], node2], *tree.open(LeafIndex::<2>::new(3).unwrap()));
}
