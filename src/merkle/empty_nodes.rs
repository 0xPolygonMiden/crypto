use super::{Felt, Rpo256, RpoDigest, Vec, Word, WORD_SIZE, ZERO};

// EMPTY NODES SUBTREES
// ================================================================================================

/// An empty sub-trees set.
///
/// Will provide precomputed node values for path computation for pre-defined depths.
pub struct EmptyNodesSubtrees;

impl EmptyNodesSubtrees {
    // PROVIDERS
    // --------------------------------------------------------------------------------------------

    /// Returns an empty subtrees set for depth `64`.
    pub const fn empty_subtrees_64() -> [RpoDigest; 65] {
        EMPTY_SUBTREES_64
    }

    /// Fetches a precomputed node for an empty sub-tree with maximum depth `64`.
    pub const fn get_node_64(depth: u8) -> RpoDigest {
        debug_assert!(depth <= 64);
        EMPTY_SUBTREES_64[depth as usize]
    }

    /// Computes a sequence of empty hashes for an arbitrary depth
    pub fn compute_empty_hashes(depth: u8) -> Vec<RpoDigest> {
        (0..depth + 1)
...
#[test]
fn constant_depth_64_is_valid() {
    let reference = EmptyNodesSubtrees::compute_empty_hashes(64);
    assert_eq!(EMPTY_SUBTREES_64.to_vec(), reference);
}
