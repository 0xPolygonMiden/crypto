use super::{Felt, RpoDigest, TieredSmt, WORD_SIZE, ZERO};

// EMPTY TIERED SUBTREES
// ================================================================================================

/// An empty tiered sub-trees set.
///
/// Will provide precomputed node values for path computation.
pub struct EmptyTieredSubtrees;

impl EmptyTieredSubtrees {
    // PROVIDERS
    // --------------------------------------------------------------------------------------------

    /// Fetches the pre-computed empty node for a given depth, falling back to default, if the
    /// queried depth is greater than `[TieredSmt::MAX_DEPTH]`.
    pub fn get_node<T>(depth: u8) -> T
    where
        RpoDigest: Into<T>,
    {
        debug_assert!(depth <= TieredSmt::MAX_DEPTH);
        // we could panic here if depth is invalid as it would be possible only for a bug on the
        // tiered merkle tree implementation, but it's not worthy to create such risk.
        EMPTY_SUBTREES
            .get(depth as usize)
            .copied()
            .unwrap_or_default()
...
/// empty.
const EMPTY_SUBTREES: [RpoDigest; 1 + TieredSmt::MAX_DEPTH as usize] = [
    RpoDigest::new([
...
fn values_correspond_to_manually_generated_tree() {
    use crate::merkle::{empty_merkle_subtrees, Vec};
