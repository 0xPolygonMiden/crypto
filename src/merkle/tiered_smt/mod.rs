use super::{
    BTreeMap, Felt, MerkleError, MerklePath, Rpo256, RpoDigest, StarkField, Vec, Word, WORD_SIZE,
    ZERO,
};
use core::borrow::Borrow;

mod constants;
pub use constants::EmptyTieredSubtrees;
...
impl TieredSmt {
    ...
    pub fn with_storage(storage: Storage) -> Result<Self, MerkleError> {
    ...
    pub fn with_leaves<I, T>(self, leaves: I) -> Result<Self, MerkleError>
    where
    ...
    pub fn get_node(&self, index: &NodeIndex) -> Result<Word, MerkleError> {
        ...
        let node = self
            .storage
            .get_node(index)?
            .unwrap_or_else(|| EmptyTieredSubtrees::get_node(index.depth()));
        Ok(node)
    }

    /// Builds a path from the node to the root.
    pub fn get_node_path(&self, mut index: NodeIndex) -> Result<MerklePath, MerkleError> {
    ...
    pub fn get_leaf_proof(&self, key: &Word) -> Result<LeafProof, MerkleError> {
        ...
        let node = self.get_leaf_index(key)?;
        let input = match self.get_type(&node)? {
            ...
            NodeType::Leaf => self
                .get_leaf_from_unchecked_node(&node)
                .map(LeafProofInput::Upper)?,
        };
    ...
    pub fn get_bottom_leaves(&self, index: u64) -> Result<Vec<Leaf>, MerkleError> {
        self.storage
    ...
    fn get_type(&self, index: &NodeIndex) -> Result<NodeType, MerkleError> {
    ...
    fn replace_upper_leaf(&mut self, index: NodeIndex, leaf: Leaf) -> Result<(), MerkleError> {
    ...
    fn append_bottom_leaf(&mut self, index: NodeIndex, leaf: Leaf) -> Result<(), MerkleError> {
        ...
        let mut ordered_leaves = self
            ...
            .unwrap_or_default();
        ordered_leaves.push(leaf);
        ordered_leaves.sort_by_key(|leaf| leaf.key);
        ordered_leaves.dedup();

        // update the storage
        let node = Self::hash_ordered_leaves(&ordered_leaves);
        ordered_leaves.iter().try_for_each(|leaf| {
            self.storage.replace_key(leaf.key, index)?;
            self.storage.replace_leaf_value(leaf.key, leaf.value)?;
            Ok(())
        })?;
    ...
    /// of the given index).
    fn update_path_to_root(&mut self, mut index: NodeIndex) -> Result<(), MerkleError> {
    ...
    /// Fetch a leaf without checking the expected node type, failing only if the storage fails.
    fn get_leaf_from_unchecked_node(&self, node: &NodeIndex) -> Result<Leaf, MerkleError> {
        ...
        match self.storage.get_leaf_value(&key)? {
            Some(value) => Ok(Leaf::new(key, value)),
