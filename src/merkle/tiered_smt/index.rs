use super::{Felt, Leaf, RpoDigest, StarkField, TieredSmt, Word, WORD_SIZE};
...
impl From<&Word> for LeafIndex {
    fn from(key: &Word) -> Self {
        CanonicalWord::from(key).into()
    }
}
...
impl LeafIndex {
    ...
    pub fn remaining_path_with_depth(&self, depth: u8) -> CanonicalWord {
        ...
        let value = (depth == TieredSmt::MAX_DEPTH)
            .then_some(0)
            .unwrap_or_else(|| (self.path() << depth) >> depth);
...
mod tests {
    ...
    proptest! {
        #[test]
        fn leaf_index_traversal_jumps_correctly(
            x in prop::num::u64::ANY,
        ) {
            // create a new max value key with the bits path filled
            let mut key = [ZERO - ONE; WORD_SIZE];
            let value = Word::default();
            key[3] = Felt::new(x);
            let x = key[3].as_int();

            // initialize the index
            let leaf = Leaf::new(key.into(), value);
            let mut index = LeafIndex::from(&leaf);

            assert_eq!(index.next_back(), None);
            assert_eq!(index.next_back(), None);
            assert_eq!(index.next(), Some(NodeIndex::new(16, x >> 48)));
            assert_eq!(index.next(), Some(NodeIndex::new(32, x >> 32)));
            assert_eq!(index.next(), Some(NodeIndex::new(48, x >> 16)));
            assert_eq!(index.next(), Some(NodeIndex::new(64, x)));
            assert_eq!(index.next(), None);
            assert_eq!(index.next(), None);
            assert_eq!(index.next_back(), Some(NodeIndex::new(64, x)));
            assert_eq!(index.next_back(), Some(NodeIndex::new(48, x >> 16)));
            assert_eq!(index.next_back(), Some(NodeIndex::new(32, x >> 32)));
            assert_eq!(index.next_back(), Some(NodeIndex::new(16, x >> 48)));
            assert_eq!(index.next_back(), None);
            assert_eq!(index.next_back(), None);

            assert_eq!(index.remaining_path(), CanonicalWord::from(&key));
            for r in [16, 32, 48, 64] {
                ...
                remaining[3] = if r > 63 {
                    ...
                    Felt::new((remaining[3].as_int() << r) >> r)
                };
                let remaining = CanonicalWord::from(&remaining);
                assert_eq!(index.remaining_path(), remaining);
            }
        }
    }
}
