...
impl Default for TieredTestEngine {
    fn default() -> Self {
        ...
        assert_eq!(root, EmptyTieredSubtrees::get_node(0));
        Self { tree }
    }
}
...
impl TieredTestEngine {
    ...
    {
        let key = Word::from(key);
        let node = self.tree.get_leaf_index(&key).unwrap();
        assert!(node.is_max_depth());
        let bottom_leaves = self.tree.get_bottom_leaves(node.index()).unwrap();
        let mut leaves: Vec<_> = leaves
            .into_iter()
            .map(|key| {
                let key = Word::from(key);
                let value = self.tree.get_leaf_value(&key).unwrap().unwrap();
                Leaf::new(key.into(), value)
            })
            .collect();
        leaves.sort_by_key(|leaf| leaf.key);
        assert_eq!(bottom_leaves, leaves);
        self
    }
        ...
        Word: From<W>,
    {
        ...
        while depth > 0 {
            ...
            let node = self
                ...
                .unwrap()
                .unwrap_or_else(|| EmptyTieredSubtrees::get_node(depth));
            path.push(node);
            depth -= 1;
        }
