use super::*;
use crate::{utils::collections::BTreeSet, StarkField, ONE, WORD_SIZE};
...
fn append_to_bottom_level_is_consistent() {
    ...
    test.insert(key_e, Word::default())
        ...
        .expect_bottom_leaves(key_d, [key_d])
        .expect_bottom_leaves(key_e, [key_e]);
    ...
    test.insert(key_f, Word::default())
        ...
        .expect_bottom_leaves(key_d, [key_d])
        .expect_bottom_leaves(key_e, [key_e, key_f]);
    */
}
...
impl Default for TieredTestEngine {
    fn default() -> Self {
        ...
        let tree = TieredSmt::default().with_storage(storage);
        let root = RpoDigest::from(*tree.root());
        assert_eq!(root, EmptyNodesSubtrees::get_node_64(0));
        Self { tree }
    }
}
...
impl TieredTestEngine {
    ...
    pub fn expect_bottom_leaves<W, L, K>(&mut self, key: W, leaves: L) -> &mut Self
    where
        Word: From<W>,
        Word: From<K>,
    ...
    {
        let key = Word::from(key);
        let index = self.tree.get_leaf_index(&key);
        assert_eq!(index.depth(), TieredSmt::MAX_DEPTH);
        let bottom_leaves = self.tree.get_bottom_leaves(index.value()).unwrap();
        let mut leaves: Vec<_> = leaves
            .into_iter()
            .map(|key| {
                let key = Word::from(key);
                let value = *self.tree.get_leaf_value(&key).unwrap();
                let key = CanonicalWord::from(key);
                (key, value)
            })
            .collect();
        leaves.sort_by_key(|x| x.0);
        let computed: Vec<_> = bottom_leaves.clone().into_iter().collect();
        ...
        Word: From<W>,
    {
        ...
        while depth > 0 {
            ...
            let index = index ^ 1;
            let node = self.tree.get_node(&NodeIndex::new(depth, index)).unwrap();
            path.push(node);
            depth -= 1;
        }
        let opening = self.tree.get_leaf_path(&key).unwrap();
        assert_eq!(opening.as_slice(), path.as_slice());
        self
    }
}
